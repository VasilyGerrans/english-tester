...
function findCommonAnchors(sentences: string[]): string[] {
...
function cleanCommonText(text: string): string {
    return text
        .replace(/\s+([.,!?;:])/g, '$1')
        .replace(/'\s+/g, "'")
        .replace(/\s+'/g, "'")
...
export function extractDifferencesMultiLCS(sentences: string[]): {
    common: string;
    answers: string[];
} {
